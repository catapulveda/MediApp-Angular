        ...
        if (!logueado) {
            ...
            if (!helper.isTokenExpired(token)) {
                ...
                return this.menuService.listarPorUsuario(decodedToken.user_name)
                    .pipe(map((rutaMenus: Menu[]) => {
                        this.menuService.menuCambio.next(rutaMenus);

                        let cont = 0;
                        for (let rutaMenu of rutaMenus) {
                            if (urlSolicitada.startsWith(rutaMenu.url)) {
                                cont++;
                                break;
                            }
                        }
